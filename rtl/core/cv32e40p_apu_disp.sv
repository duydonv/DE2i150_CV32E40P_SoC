...
module cv32e40p_apu_disp (
  ...
  // Quartus Lite (SV-2005) does not accept the inline `for (genvar i = ...)`
  // construct. Declare genvars at module scope instead.
  genvar gvr;
  generate
    for (gvr = 0; gvr < 3; gvr = gvr + 1) begin : gen_read_deps
      assign read_deps_req[gvr]      = (read_regs_i[gvr] == addr_req) & read_regs_valid_i[gvr];
      assign read_deps_inflight[gvr] = (read_regs_i[gvr] == addr_inflight) & read_regs_valid_i[gvr];
      assign read_deps_waiting[gvr]  = (read_regs_i[gvr] == addr_waiting) & read_regs_valid_i[gvr];
    end
  endgenerate
  ...
  generate
    for (gvw = 0; gvw < 2; gvw = gvw + 1) begin : gen_write_deps
      assign write_deps_req[gvw]      = (write_regs_i[gvw] == addr_req) & write_regs_valid_i[gvw];
      assign write_deps_inflight[gvw] = (write_regs_i[gvw] == addr_inflight) & write_regs_valid_i[gvw];
      assign write_deps_waiting[gvw]  = (write_regs_i[gvw] == addr_waiting) & write_regs_valid_i[gvw];
    end
  endgenerate
