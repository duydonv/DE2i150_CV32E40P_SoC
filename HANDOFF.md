...
- Đã thêm firmware benchmark riêng (`make benchmark`) để đo `mcycle` và
  `minstret` cho hai nhóm: `MAC+ReLU+Clamp` và
  `Dot4_acc+ReLU+Clamp+p.lw+lp.setup`. Benchmark hiện in kết quả dạng
  CSV qua UART TX 115200 8N1; LEDR[7:0] chỉ còn báo trạng thái ngắn.
...
1. **Viết benchmark hiệu năng nhỏ**:
   ...
   - ✅ Hai kernel đầu tiên: `MAC+activation` và `streaming dot4`.
...
- UART TX 115200 8N1 qua DB9 RS-232 để in benchmark CSV.
