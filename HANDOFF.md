...
- Đã thêm GUI MVP `firmware/tflm_mnist_uart_gui.py` bằng PySide6 + pyqtgraph,
  ...
  preview, chạy inference một sample được chọn mỗi lần, biểu đồ score ref/opt,
  cycles/speedup, true-label match, expected-artifact match và UART log.
...
| MNIST UART PySide6 GUI MVP | ✅ smoke-tested | `firmware/tflm_mnist_uart_gui.py`, `firmware/gui_requirements.txt`; chọn port/baud, ping, preview, score bars, log UART; hiện chạy một selected sample mỗi lần |
